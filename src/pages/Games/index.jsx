...
function SkeletonCard() {
  return (
    <div className="bg-white-40 rounded-2xl shadow-lg p-4 w-[300px] animate-pulse">
      ...
      >
        {loading ? (
          ...
          filteredItems.map((item, i) => (
            <motion.div key={`${filter ? "active" : "inactive"}-${i}`} variants={itemVariants}>
              <CodeCard
                data={item}
                onCopy={onCopy}
...
function GamesList({ topGames, allGames }) {
  ...
  return (
    <>
      <motion.h3
        className="mb-3 text-xl font-semibold mt-8 text-cyan-600"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        {topGames.map(({ name, time, img, url }) => (
          <motion.div key={name} className="text-center" variants={itemVariants}>
            <Tippy
      ...
      <motion.h3
        className="mb-3 text-xl font-semibold mt-10 text-cyan-600"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
...
function GamesDashboard({
  ...
  return (
    <motion.div
      className="bg-slate-50 p-8 rounded-2xl shadow-md mt-10"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
              ...
              >
                {loading ? (
                  ...
                  paginatedItems.map((item) => (
                    <motion.div key={`event-${item.id}`} variants={itemVariants}>
                      <EventCard
                        data={item}
